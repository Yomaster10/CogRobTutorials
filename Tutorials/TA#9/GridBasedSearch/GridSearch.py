# This code was partly written by Oren Salzman and Dean Zadok, partly by Niv Ostroff, and revised by Yotam Granov
...
class AStarPlanner(object):
    ...
    def plan(self):
        ...
        self.open[initial_state] = (self.heuristic(env, initial_state, 0, h_weight),env.start)  # append initial state and 'distance' to goal
        while self.open:
            ...
            if self.is_final_state(current_state):
                ...
                for neighbour_state in self.state_neighbors(current_state):
                    if neighbour_state is not None:
                        new_g = self.gList[current_state] + self.cost_to_go(current_state, neighbour_state)
                    if (neighbour_state not in self.open) and (neighbour_state not in self.close) and (
                        ...
                        curr_heuristic, _ = self.open[neighbour_state]
                        ...
                        curr_heuristic, _ = self.close[neighbour_state]
    ...
    def cost_to_go(self, state,neighbor_state):
    ...
    def list_path(self, parent, end_state):
        '''
        Create paths with positional values
        '''
        path = []
